...
threshold_x_values = np.linspace(0.3,0.6,thn)
shiftS_values = np.linspace(-0.8,-0.3,sSn)
...
def main(parameter_combinations, n, complete_graph):
    ...
    for param in parameter_combinations:
        alphas = np.array([param[0],param[1]])
        threshold_x=param[2]
        shiftS = param[3]
        r=param[4]
...
if __name__ == "__main__":
    run = args.Run
    alphas = list(itertools.product(alpha1s,alpha2s))
    ...
    df = pd.DataFrame(data, columns=['alphas',
                                     ...
                                     'periodic',
                                     'limit'])
    csvname = 'DataFiles/Periodicity2Result{}.csv'.format(args.Run)
