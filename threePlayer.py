def _run_single_sim(args):
    a1, a2, a3, sim_kwargs = args
    
    n = 3
    t_max = sim_kwargs['t_max']
    threshold_x = sim_kwargs['threshold_x']
    
    # 3-player alpha array
    alpha_arr = np.array([a1, a2, a3], dtype=float)
    ...
    while t_current < t_max and events_triggered < max_events:
        ...
        if sol.status == 1: 
            ...
            # for efficiency If all 3 players have acted, we are done
            if np.all(has_acted):
                break
...
# sweep
def generate_3player_heatmap_csv(
    grid_size: int = 100, 
    t_max: float = 5000,
    save_path: str = "alpha_sweep_3player_fixed_a3.csv"
):
    alpha_vals = np.round(np.linspace(-1.0, 1.0, grid_size), 6)
    
    # eg. alpha_3 values
    alpha3_list = [-0.2, 0.0, 0.2, 0.4, 0.8, 1.0]
    ...
    # Generate unique pairs for alpha_1 and alpha_2
    unique_pairs = list(itertools.combinations_with_replacement(alpha_vals, 2))
    
    # Build the list covering all unique pairs across all fixed alpha_3 values
    tasks = [(a1, a2, a3, sim_kwargs) for a3 in alpha3_list for a1, a2 in unique_pairs]

    print(f"Starting {len(tasks)} simulations across {len(alpha3_list)} slices")
    ...
    for res in results:
        ...
        # Mirror the data to reconstruct the full grid
    ...
    # Sort 
    df = df.sort_values(by=["alpha_3", "alpha_1", "alpha_2"]).reset_index(drop=True)
    
    out = Path(save_path)
    out.parent.mkdir(parents=True, exist_ok=True)
    df.to_csv(out, index=False)
    
    total_grids = len(alpha3_list)
    print(f"Saved {total_grids} full {grid_size}x{grid_size} grids ({len(df)} rows) to: {out.resolve()}")
    return df